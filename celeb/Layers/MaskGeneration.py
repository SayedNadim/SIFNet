...
class Masks():
    ...
    def get_ca_mask(h, w, scale=None, r=None):
        ...
        if r is None:
            r = random.randint(2, 4)  # repeat median filter r times
    ...
    def get_random_mask(h, w):
        f = random.choice([Masks.get_ff_mask, Masks.get_ca_mask])
        # f = random.choice([Masks.get_ca_mask, Masks.get_ff_mask])
        return f(h, w)
